...
class AutoTrader(BaseAutoTrader):
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.FUTURE:
            ...
            if bid_prices[0] != 0:
                if np.average(bid_prices) >= self.avg_bid_prices and np.average(ask_prices) >= self.avg_ask_prices: # Bull market
                    new_bid_price = bid_prices[0] - TICK_SIZE_IN_CENTS
                    new_ask_price = ask_prices[0] + (TICK_SIZE_IN_CENTS * 2)
                elif np.average(bid_prices) < self.avg_bid_prices and np.average(ask_prices) < self.avg_ask_prices: # Bear market
            ...
            # Save average bid & ask prices
            self.avg_bid_prices = np.average(bid_prices)
            self.avg_ask_prices = np.average(ask_prices)

            # Send cancel orders if needed
